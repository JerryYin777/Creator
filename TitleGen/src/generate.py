...
def generate(model,tokenizer,config:ValidConfig,content):
    ...
    input_ids=paddle.tensor(input_ids).view(1,-1).to(config.device)
    token_type_ids=paddle.tensor(token_type_ids).view(1,-1).long().to(config.device)
    title_ids=[]
    with paddle.no_grad():
        for _ in range(config.max_generate_len):
            ...
            input_ids=paddle.concat([input_ids,token_id.view(1,1)],-1)
            token_type_ids=paddle.concat([token_type_ids,paddle.tensor(title_token).view(1,1).long().to(config.device)],-1)
    result=""
...
if __name__=='__main__':
    ...
    model.load_state_dict(paddle.load('./check/model_finetune.pdparam'))
